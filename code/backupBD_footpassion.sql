...
You do not have the SUPER privilege and binary logging is enabled (you *might* want to use the less safe log_bin_trust_function_creators variable)	0,063 sec
...
CREATE PROCEDURE `finpartido` (IN `a` INT)   begin
	if (
    ...
    (select goles_local from partidos where partidos.cod=a)
       ...
       then update partidos 
        ...
        update equipos
        set win=win+1 where id in (select * from (select e_visitante from partidos where partidos.cod=a) as TempP);
        update equipos
        set loose=loose+1 where id in (select * from (select e_local from partidos where partidos.cod=a) as TempP);
        update equipos
        set total=(win*3)+(draw*1) where id in (select * from (select e_local from partidos where partidos.cod=a) as TempP);
		update equipos
		set total=(win*3)+(draw*1) where id in (select * from (select e_visitante from partidos where partidos.cod=a) as TempP);
    end if;
    ...
    (select goles_visitante from partidos where partidos.cod=a)
       ...
       update partidos 
        ...
        update equipos
        set win=win+1 where id in (select * from (select e_local from partidos where partidos.cod=a) as TempP);
        update equipos
        set loose=loose+1 where id in (select * from (select e_visitante from partidos where partidos.cod=a) as TempP);
        update equipos
        set total=(win*3)+(draw*1) where id in (select * from (select e_local from partidos where partidos.cod=a) as TempP);
		update equipos
		set total=(win*3)+(draw*1) where id in (select * from (select e_visitante from partidos where partidos.cod=a) as TempP);
    ...
    (select goles_visitante from partidos where partidos.cod=a)
       ...
       update partidos 
        ...
        update equipos
        set draw=draw+1 where id in (select * from (select e_local from partidos where partidos.cod=a) as TempP);
        update equipos
        set draw=draw+1 where id in (select * from (select e_visitante from partidos where partidos.cod=a) as TempP);
        update equipos
        set equipos.total=(equipos.win*3)+(equipos.draw*1) where id in (select * from (select e_visitante from partidos where partidos.cod=a) as TempP);
		update equipos
		set equipos.total=(equipos.win*3)+(equipos.draw*1) where id in (select * from (select e_local from partidos where partidos.cod=a) as TempP);
	end if;
end$$

CREATE PROCEDURE `ingreso` ()   begin

		
end$$

CREATE PROCEDURE `last_partidos` (IN `a` VARCHAR(50))   begin

select partidos.fecha, partidos.hora, partidos.jornada, 
(select nombre_e from equipos inner join partidos on id=e_local limit 1) as 'local',
(select nombre_e from equipos inner join partidos on id=e_visitante limit 1) as 'visitante'
from equipos inner join partidos on id=e_local or id=e_visitante
...
select partidos.fecha, partidos.hora, partidos.jornada, nombre_e, nombre_e
from equipos inner join partidos on id=e_local or id=e_visitante
where (select nombre_e from equipos inner join partidos on id=e_local) like "%Celta%" or (select nombre_e from equipos inner join partidos on id=e_visitante) like "%Celta%"
...
CREATE PROCEDURE `ver_equipo` (IN `a` VARCHAR(50))   begin
	select * from equipos
    where nombre_e like a;
end$$

CREATE PROCEDURE `ver_jugadores` (IN `a` VARCHAR(50))   begin
	select jugadores.id as 'cod', nombre_j as 'Nombre', valor_mercado as 'valor', posicion, dorsal,amarillas,sancion,lesion
    from jugadores 
...
INSERT INTO `arbitros` (`id`, `nombre_a`) VALUES
...
INSERT INTO `equipos` (`id`, `nombre_e`, `abreviatura`, `nombre_l`, `presupuesto_e`, `win`, `loose`, `draw`, `total`, `estadio`, `entrenador`, `presidente`, `escudo`) VALUES
(1, 'Real Club Celta de Vigo', 'RCC', 'Liga Santander', 40000, 0, 0, 1, 1, 'Abanca Balaídos', NULL, NULL, 'img/escudos/1.gif'),
(2, 'Real Madrid', 'RM', 'Liga Santander', 10000000, 1, 0, 0, 3, 'Santiago Bernabeu', NULL, NULL, 'img/escudos/2.png'),
(3, 'Fútbol Club Barcelona', 'FCB', 'Liga Santander', 0, 0, 0, 1, 1, 'Spotify Camp Nou', NULL, NULL, 'img/escudos/3.png'),
(4, 'Atlético de Madrid', 'ATM', 'Liga Santander', 500000, 1, 0, 0, 3, 'Wakanda Metropolitano', NULL, NULL, 'img/escudos/4.png'),
(5, 'Villareal Club de Fútbol', 'VLL', 'Liga Santander', 50500, 1, 0, 0, 3, 'Estadio de la Cerámica', NULL, NULL, 'img/escudos/5.gif'),
(6, 'Club Atlético Osasuna', 'OSA', 'Liga Santander', 50500, 1, 0, 0, 3, 'El Sadar', NULL, NULL, 'img/escudos/6.gif'),
(7, 'Cádiz CF', 'CCF', 'Liga Santander', 50500, 0, 1, 0, 0, 'Estadio nuevo Mirandilla', NULL, NULL, 'img/escudos/7.gif'),
(8, 'Elche Club de Fútbol', 'ELC', 'Liga Santander', 40500, 0, 1, 0, 0, 'Estadio Martínez Valero', NULL, NULL, 'img/escudos/8.gif'),
(9, 'Getafe CF', 'GCF', 'Liga Santander', 50500, 0, 1, 0, 0, 'Coliseum Alfonso Pérez', NULL, NULL, 'img/escudos/9.gif'),
(10, 'Rayo Vallecano', 'RVA', 'Liga Santander', 13500, 0, 0, 1, 1, 'Estadio de Vallecas', NULL, NULL, 'img/escudos/10.gif'),
(11, 'RCD Espanyol', 'ESY', 'Liga Santander', 8500, 0, 0, 1, 1, 'RCDE Stadium', NULL, NULL, 'img/escudos/11.gif'),
(12, 'RCD Mallorca', 'CDM', 'Liga Santander', 3500, 0, 0, 1, 1, 'Visit Mallorca Estadi', NULL, NULL, 'img/escudos/12.gif'),
(13, 'Real Betis', 'RBB', 'Liga Santander', 9500, 1, 0, 0, 3, 'Estadio Benito Villamarín', NULL, NULL, 'img/escudos/13.gif'),
(14, 'Real Sociedad', 'RSO', 'Liga Santander', 13500, 1, 0, 0, 3, 'Reale Arena', NULL, NULL, 'img/escudos/14.gif'),
(15, 'Sevilla CF', 'SCF', 'Liga Santander', 15500, 0, 1, 0, 0, 'Ramón Sánchez Pizjuán', NULL, NULL, 'img/escudos/15.gif'),
(16, 'Valencia CF', 'VCF', 'Liga Santander', 20500, 1, 0, 0, 3, 'Camp de Mestalla', NULL, NULL, 'img/escudos/16.gif'),
(17, 'Atletic Club de Bilbao', 'ACB', 'Liga Santander', 20500, 0, 0, 1, 1, 'San Mamés', NULL, NULL, 'img/escudos/17.gif'),
(18, 'Almería FC', 'ALM', 'Liga Santander', 20500, 0, 1, 0, 0, 'Estadio de los juegos mediterráneos', 'Joan Francesc Ferrer', 'Turki Alalshikh', 'img/escudos/18.gif'),
(19, 'Real Valladolid CF', 'RVA', 'Liga Santander', 20500, 0, 1, 0, 0, 'Estadio municipal José Zorrilla', NULL, NULL, 'img/escudos/19.gif'),
(20, 'Girona FC FC', 'GFC', 'Liga Santander', 20500, 0, 1, 0, 0, 'Estadio Municipal de Montilivi', NULL, NULL, 'img/escudos/20.gif');
...
CREATE TRIGGER `fichaje` BEFORE INSERT ON `fichajes` FOR EACH ROW begin
		if new.precio > (select presupuesto_e from equipos where id = new.equipo_next)
			then
			signal sqlstate '45000' 
			set message_text = "A Este equipo no le da la pasta xdxd";
        elseif new.precio < (select presupuesto_e from equipos where id = new.equipo_next)
			then
			update jugadores
				set equipo_actual = new.equipo_next where jugadores.id=new.jugador;
				
			if new.tipo like "traspaso"
				then 
				update equipos
				set presupuesto_e = presupuesto_e - new.precio where id like new.equipo_next
				;
			end if ;
		end if;
end
...
CREATE TABLE `goles` (
  ...
  `id` int NOT NULL,
  `tipo` enum('falta','cabeza','volea','tiro','p.p','penalty','normal') DEFAULT NULL,
...
INSERT INTO `goles` (`n_licencia_j`, `cod_partido`, `id`, `tipo`, `minuto`, `asistencia`) VALUES
(139, 1, 1, 'tiro', '9', 137),
(344, 1, 2, 'tiro', '11', 339),
...
CREATE TRIGGER `gol` AFTER INSERT ON `goles` FOR EACH ROW begin
	...
	if ((select equipo_actual from jugadores  where jugadores.id=new.n_licencia_j) 
    like 
...
CREATE TABLE `jugadores` (
  ...
  `foto_jug` char(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ;
...
INSERT INTO `jugadores` (`id`, `nombre_j`, `valor_mercado`, `posicion`, `equipo_actual`, `dorsal`, `amarillas`, `sancion`, `lesion`) VALUES
...
CREATE TRIGGER `lesion` AFTER INSERT ON `lesiones` FOR EACH ROW begin
		update jugadores
		set lesion="si" where new.n_licencia_j=jugadores.id
        ;
end
$$
DELIMITER ;
...
CREATE TRIGGER `sancion` AFTER INSERT ON `sanciones` FOR EACH ROW begin
		update jugadores
		set sancion="si" where new.n_licencia_j=jugadores.id
        ;
end
$$
DELIMITER ;
...
CREATE TRIGGER `acumulacion` AFTER INSERT ON `tarjetas` FOR EACH ROW begin
	...
	if (new.tipo like "amarilla") and (select amarillas from jugadores where id=new.n_licencia_j)=5
    then 
        ...
        ;
        update jugadores
        set amarillas=0 where id=new.n_licencia_j
        ;
    ...
    then
		...
		update jugadores
        set amarillas=amarillas+1 where id=new.n_licencia_j
        ;
...
CREATE TABLE `usuarios` (
  ...
  `tipo` enum("periodista", "vendedor", "admin", "normal") DEFAULT NULL,
  `foto` char(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ;
...
INSERT INTO `usuarios` (`nombre`, `localidad`, `correo`, `password`,`tipo`,`foto`) VALUES
('Alex','Vigo', 'ale@hola.org', 'abc123.', "normal", "img/usuarios/1.jpg" ),
('Carlos','Vigo', 'carlos@hola.org', 'abc123.', "admin", "img/usuarios/2.jpg" ),
('Victor','Vigo', 'victor@hola.org', 'abc123.', "periodista", "img/usuarios/3.jpg" ),
('Raul','Vigo', 'Raul@hola.org', 'abc123.', "vendedor", "img/usuarios/4.jpg" );
...
INSERT INTO `productos` (`titulo`, `precio`, `descripcion`, `IdUsuario`, `foto`, `keywords`, `fecha`) VALUES
('Camiseta españa azul', 200, "El mejor coche", 1, 'img/productos/1.png', 'camisetas merch', "2020-5-12"),
('Camiseta De Bruyne', 300, "Marioneta", 2, 'img/productos/2.png', 'camisetas merch', "2020-5-12"),
('Camiseta Brasil', 200, "El mejor coche", 1, 'img/productos/3.png', 'camisetas bruyne', "2020-5-12"),
('Camiseta Barcaaaa', 200, "El mejor coche", 1, 'img/productos/4.png', 'camisetas ', "2020-5-12"),
('Botas negras', 200, "El mejor coche", 1, 'img/productos/5.png', 'botas merch', "2020-5-12"),
('Botas azules', 200, "El mejor coche", 1, 'img/productos/6.png', 'botas merch', "2020-5-12"),
('Botas rosas', 200, "El mejor coche", 1, 'img/productos/7.png', 'botas merch', "2020-5-12"),
('Pelota españa', 200, "El mejor coche", 1, 'img/productos/8.png', ' merch', "2020-5-12");
-- --------------------------------------------------------
...
CREATE TABLE `noticias` (
  `id` int  PRIMARY KEY NOT NULL  AUTO_INCREMENT,
  `resumen` char(40) NOT NULL,
  `titulo` char(30) NOT NULL,
  `foto` char(100) DEFAULT NULL,
  `IdUsuario` int DEFAULT null,
  `cuerpo` char(200) NOT NULL,
  `keywords` char(200) DEFAULT NULL,
  `fecha` date DEFAULT NULL,
  FOREIGN KEY (`IdUsuario`) REFERENCES `usuarios` (`IdUsuario`)
  ON DELETE CASCADE
...
INSERT INTO `noticias` (`titulo`, `cuerpo`, `resumen`, `IdUsuario`, `foto`, `keywords`, `fecha`) VALUES
('SuperCoche1', 'ourhfvñoue', "El mejor coche", 1, 'img/noticias/1.png', 'españa internacional', "2020-5-12"),
('SuperCoche2', 'ourhfvñoue', "El mejor coche", 2, 'img/noticias/2.png', 'españa internacional', "2020-5-12"),
('SuperCoche3', 'ourhfvñoue', "El mejor coche", 3, 'img/noticias/3.png', 'españa', "2020-5-12"),
('SuperCoche4', 'ourhfvñoue', "El mejor coche", 4, 'img/noticias/4.png', 'uefa ', "2020-5-12"),
('Marioneta5', 'ihsadbcvlsaidhcbal', "Marioneta", 1, 'img/noticias/5.png', 'españa internacional uefa', "2020-5-12"),
('SuperCoche6', 'ourhfvñoue', "El mejor coche", 2, 'img/noticias/6.png', 'internacional uefa', "2020-5-12"),
('SuperCoche7', 'ourhfvñoue', "El mejor coche", 3, 'img/noticias/7.png', 'españa internacional madrid atletico', "2020-5-12"),
('SuperCoche8', 'ourhfvñoue', "El mejor coche", 4, 'img/noticias/8.png', '', "2020-5-12"),
('SuperCoche9', 'ourhfvñoue', "El mejor coche", 1, 'img/noticias/9.png', 'españa internacional danjuma', "2020-5-12");
...
--
ALTER TABLE `fichajes`
  ADD CONSTRAINT `fichajes_ibfk_1` FOREIGN KEY (`jugador`) REFERENCES `jugadores` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE,
  
  ADD CONSTRAINT `fichajes_ibfk_2` FOREIGN KEY (`equipo_next`) REFERENCES `equipos` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE;
...
--
ALTER TABLE `goles`
  ADD CONSTRAINT `goles_ibfk_1` FOREIGN KEY (`n_licencia_j`) REFERENCES `jugadores` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE,
  
  ADD CONSTRAINT `goles_ibfk_2` FOREIGN KEY (`asistencia`) REFERENCES `jugadores` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE;
...
--
ALTER TABLE `jugadores`
  ADD CONSTRAINT `jugadores_ibfk_1` FOREIGN KEY (`equipo_actual`) REFERENCES `equipos` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE;
...
--
ALTER TABLE `lesiones`
  ADD CONSTRAINT `lesiones_ibfk_1` FOREIGN KEY (`n_licencia_j`) REFERENCES `jugadores` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE;
...
--
ALTER TABLE `partidos`
  ADD CONSTRAINT `partidos_ibfk_1` FOREIGN KEY (`e_local`) REFERENCES `equipos` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE,
  
  ADD CONSTRAINT `partidos_ibfk_2` FOREIGN KEY (`e_visitante`) REFERENCES `equipos` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE,
  
  ADD CONSTRAINT `partidos_ibfk_3` FOREIGN KEY (`arbitro`) REFERENCES `arbitros` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE;
...
--
ALTER TABLE `sanciones`
  ADD CONSTRAINT `sanciones_ibfk_1` FOREIGN KEY (`n_licencia_j`) REFERENCES `jugadores` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE,
...
--
ALTER TABLE `tarjetas`
  ADD CONSTRAINT `tarjetas_ibfk_1` FOREIGN KEY (`n_licencia_j`) REFERENCES `jugadores` (`id`)
  ON DELETE cascade
  ON UPDATE CASCADE,
...
drop user if exists 'normal'@'localhost','vendedor'@'localhost','periodista'@'localhost';
create user 'normal'@'localhost' IDENTIFIED BY 'normal123.';
create user 'vendedor'@'localhost' IDENTIFIED BY 'vendedor123.';
create user 'periodista'@'localhost' IDENTIFIED BY 'periodista123.';

GRANT SELECT on grupo10.* TO 'normal'@'localhost';
GRANT INSERT, DELETE, UPDATE on grupo10.usuarios TO 'normal'@'localhost';

GRANT SELECT on grupo10.* TO 'periodista'@'localhost';
GRANT SELECT, INSERT, UPDATE, DROP, DELETE on grupo10.jugadores TO 'periodista'@'localhost';
GRANT SELECT, INSERT, UPDATE, DROP, DELETE on grupo10.ligas TO 'periodista'@'localhost';
GRANT SELECT, INSERT, UPDATE, DROP, DELETE on grupo10.goles TO 'periodista'@'localhost';
GRANT SELECT, INSERT, UPDATE, DROP, DELETE on grupo10.equipos TO 'periodista'@'localhost';
GRANT SELECT, INSERT, UPDATE, DROP, DELETE on grupo10.partidos TO 'periodista'@'localhost';
GRANT SELECT, INSERT, UPDATE, DROP, DELETE on grupo10.tarjetas TO 'periodista'@'localhost';
GRANT SELECT, INSERT, UPDATE, DROP, DELETE on grupo10.sanciones TO 'periodista'@'localhost';
GRANT SELECT, INSERT, UPDATE, DROP, DELETE on grupo10.fichajes TO 'periodista'@'localhost';
GRANT SELECT, INSERT, UPDATE, DROP, DELETE on grupo10.arbitros TO 'periodista'@'localhost';
GRANT SELECT, INSERT, UPDATE, DROP, DELETE on grupo10.noticias TO 'periodista'@'localhost';
GRANT update, insert, delete on grupo10.usuarios TO 'periodista'@'localhost';
GRANT EXECUTE ON PROCEDURE grupo10.finpartido TO 'periodista'@'localhost';


GRANT SELECT, UPDATE, DELETE, DROP, INSERT on grupo10.productos TO 'vendedor'@'localhost';
GRANT SELECT on grupo10.* TO 'vendedor'@'localhost';
GRANT update, delete, insert on grupo10.usuarios TO 'vendedor'@'localhost';
